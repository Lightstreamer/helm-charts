...
Once defined, reference the entry by its name wherever a `keystoreRef` or `truststoreRef` is required — for example in a server socket [`sslConfig`](charts/lightstreamer/values.yaml#L899), the JMX [`rmiConnector`](charts/lightstreamer/values.yaml#L1976), or a Proxy Adapter [`sslConfig`](charts/lightstreamer/values.yaml#L4096), or t.

### Global socket

The [`globalSocket`](charts/lightstreamer/values.yaml#L1152) section defines timeout and size limits that apply to every server socket. The defaults are tuned for general-purpose deployments; for internet-facing scenarios, tighten read and handshake timeouts to reclaim threads from slow or abusive clients faster, and lower the request size limit to reject oversized payloads early. WebSocket frame sizing can also be adjusted here to balance memory usage against fragmentation overhead.
...
See the [`globalSocket`](charts/lightstreamer/values.yaml#L1152) section of `values.yaml` for full details.

### Security

The [`security`](charts/lightstreamer/values.yaml#L1215) section controls CORS policy and server identification behaviour.
...
See the [`security`](charts/lightstreamer/values.yaml#L1215) section of `values.yaml` for full details.
...
See the [`logging`](charts/lightstreamer/values.yaml#L1389) section of `values.yaml` for full details.

#### Primary loggers

The [`logging.loggers`](charts/lightstreamer/values.yaml#L1430) section defines the primary loggers used by the Lightstreamer Broker. The main logger is [`lightstreamerLogger`](charts/lightstreamer/values.yaml#L1492), which captures all major Broker activity. Two monitor loggers — [`lightstreamerMonitorText`](charts/lightstreamer/values.yaml#L1452) and [`lightstreamerMonitorTAB`](charts/lightstreamer/values.yaml#L1465) — emit periodic statistics in text and tabular formats. Dedicated loggers cover health checks ([`lightstreamerHealthCheck`](charts/lightstreamer/values.yaml#L1710)) and Proxy Adapter activity ([`lightstreamerProxyAdapters`](charts/lightstreamer/values.yaml#L1727)).

Each logger accepts a `level` (`OFF`, `FATAL`, `ERROR`, `WARN`, `INFO`, `DEBUG`, `TRACE`) and an `appenders` list referencing entries from [`logging.appenders`](charts/lightstreamer/values.yaml#L1394).
...
The [`logging.loggers.lightstreamerLogger.subLoggers`](charts/lightstreamer/values.yaml#L1506) section allows you to define logging levels for subloggers of `lightstreamerLogger`. Subloggers inherit appenders from their parent logger.
...
To define additional loggers, add entries to the [`extraLoggers`](charts/lightstreamer/values.yaml#L1866) section. This is useful for custom logging requirements.
...
The [`logging.appenders`](charts/lightstreamer/values.yaml#L1394) section defines the appenders available for use by loggers. The default configuration includes:

- [`dailyRolling`](charts/lightstreamer/values.yaml#L1399): A daily rolling file appender
- [`console`](charts/lightstreamer/values.yaml#L1425): A console appender
...
The [`management`](charts/lightstreamer/values.yaml#L1875) section covers operational concerns: JMX access for monitoring and management, the built-in Monitoring Dashboard, health check configuration, and various thresholds for Adapter call and thread pool monitoring. The sub-sections below cover the most commonly customized areas.
...
To customize the RMI Connector listening port, set [`management.jmx.rmiConnector.port`](charts/lightstreamer/values.yaml#L1982):
...
The chart auto-discovers which JMX connectors are enabled and includes their ports. The resulting Service is named `<fullname>-management` and defaults to `ClusterIP`, keeping management traffic internal to the cluster. See the [`management.jmx.service`](charts/lightstreamer/values.yaml#L1945) section of `values.yaml` for full details.
...
To enable TLS/SSL communication, turn on the optional [`management.jmx.rmiConnector.port.enableSsl`](charts/lightstreamer/values.yaml#L1991) flag and reference a keystore through [`management.jmx.rmiConnector.keystoreRef`](charts/lightstreamer/values.yaml#L2068) (as already explained in the [_TLS/SSL_](#tlsssl)):
...
See the [`management.jmx.rmiConnector.sslConfig`](charts/lightstreamer/values.yaml#L2059) section of `values.yaml` for full details. 
...
Then, populate the [`management.jmx.rmiConnector.credentialSecrets`](charts/lightstreamer/values.yaml#L2128) list with the references to the secrets (public access is already disabled by default through the [`management.jmx.rmiConnector.enablePublicAccess`](charts/lightstreamer/values.yaml#L2117) flag).
...
See the [`management.jmx.rmiConnector`](charts/lightstreamer/values.yaml#L1976) section of `values.yaml` for full details.
...
See the [`management.dashboard`](charts/lightstreamer/values.yaml#L2213) section of `values.yaml` for full details.

#### Health check

The [`management.healthCheck`](charts/lightstreamer/values.yaml#L2312) section configures the `/lightstreamer/healthcheck` endpoint, which load balancers and Kubernetes probes use to verify server responsiveness. The endpoint always returns `OK\r\n` (unless overridden via JMX).
...
See the [`management.healthCheck`](charts/lightstreamer/values.yaml#L2312) section of `values.yaml` for full details.

### Push session

The [`pushSession`](charts/lightstreamer/values.yaml#L2328) section tunes HTTP streaming and session behaviour.
...
See the [`pushSession`](charts/lightstreamer/values.yaml#L2328) section of `values.yaml` for full details.

### Mobile push notifications

The [`mpn`](charts/lightstreamer/values.yaml#L2733) section enables the Lightstreamer Mobile Push Notifications module, which bridges item subscriptions with Apple APNs and Google FCM so clients receive push notifications when the app is not in the foreground.
...
See the [`mpn`](charts/lightstreamer/values.yaml#L2733) section of `values.yaml` for full details.

### Web server

The [`webServer`](charts/lightstreamer/values.yaml#L3180) section controls the Lightstreamer built-in static file server, which is enabled by default and serves files from the `../pages` directory relative to the configuration folder.
...
- **Mount a volume** — use [`pagesVolume`](charts/lightstreamer/values.yaml#L3208) to mount a volume containing your static resources (HTML pages, CSS, JavaScript, images, etc.). Define the volume in `deployment.extraVolumes` and reference it by name:
...
See the [`webServer`](charts/lightstreamer/values.yaml#L3180) section of `values.yaml` for full details.

### Cluster

The [`cluster`](charts/lightstreamer/values.yaml#L3271) section configures multi-instance deployments where several Lightstreamer replicas run behind a load balancer.

Setting [`cluster.controlLinkAddress`](charts/lightstreamer/values.yaml#L3292) tells each replica which address to return in the control link response so the client SDK can reach it directly for all subsequent requests. When the load balancer provides sticky sessions, this setting can be omitted. See [Session affinity approaches](#session-affinity-approaches) for configuration examples.

Setting [`cluster.maxSessionDurationMinutes`](charts/lightstreamer/values.yaml#L3333) bounds session lifetime — when the limit is reached, the session closes gracefully, allowing the next session to be assigned to a different replica. This is particularly useful in combination with [autoscaling](#autoscaling).
...
- [`load.maxSessions`](charts/lightstreamer/values.yaml#L3347): caps the total number of concurrent client sessions. Unset by default (unlimited). Set a limit as a safety ceiling against overload.
- [`load.serverPoolMaxSize`](charts/lightstreamer/values.yaml#L3451) / [`load.serverPoolMaxQueue`](charts/lightstreamer/values.yaml#L3482): the `SERVER` pool handles client request processing, including potentially blocking Adapter calls. Increase `maxSize` (default: `1000`) if you see thread starvation under load; lower `serverPoolMaxQueue` (default: `100`) to shed load earlier rather than queue up.
- [`load.acceptPoolMaxSize`](charts/lightstreamer/values.yaml#L3496): the `ACCEPT` pool handles parsing of incoming client requests. Defaults to the number of available cores.
- [`load.eventsPoolSize`](charts/lightstreamer/values.yaml#L3404): the `EVENTS` pool dispatches update events received from Data Adapters to client sessions. Defaults to the number of available cores.
- [`load.pumpPoolSize`](charts/lightstreamer/values.yaml#L3420): the `PUMP` pool integrates update events for each session and creates update commands for clients. Defaults to the number of available cores.
- [`load.handshakePoolSize`](charts/lightstreamer/values.yaml#L3524): the `TLS-SSL HANDSHAKE` pool handles TLS/SSL handshakes on HTTPS listening sockets not configured to request a client certificate. Defaults to half the number of available cores. Only relevant when at least one server is configured with `enableHttps: true`. When client certificate authentication is enabled on a socket, the separate [`httpsAuthPoolMaxSize`](charts/lightstreamer/values.yaml#L3556) pool is used instead.
- [`load.selectorPoolSize`](charts/lightstreamer/values.yaml#L3381): number of NIO selectors (each with its own thread) sharing the same I/O operation. Defaults to the number of available cores.

See the [`load`](charts/lightstreamer/values.yaml#L3336) section of `values.yaml` for full details on all available settings.
...
To define an Adapter Set, add a new configuration to the [`adapters`](charts/lightstreamer/values.yaml#L3635) section with the following mandatory settings:

- [`id`](charts/lightstreamer/values.yaml#L3646): A unique id for the Adapter Set
- [`metadataProvider`](charts/lightstreamer/values.yaml#L3717): A Metadata Adapter configuration
- [`dataProviders`](charts/lightstreamer/values.yaml#L4380): One or more Data Adapter configurations

Moreover, set the [`enabled`](charts/lightstreamer/values.yaml#L3641) flag to `true` to include the Adapter Set in the deployment.
...
Additional optional settings are available for each Adapter Set — see [`adapterSetPool`](charts/lightstreamer/values.yaml#L3693) to configure a dedicated thread pool, and [`enableMetadataInitializedFirst`](charts/lightstreamer/values.yaml#L3714) (defaults to `true`) to control the initialization order of Metadata and Data Adapters.
...
Adapter Sets can be provisioned using different methods, configured through the [`provisioning`](charts/lightstreamer/values.yaml#L3653) section:
...
1. Embed the Adapter Set's resources in the image
   ...
   - Configure the [`provisioning.fromPathInImage`](charts/lightstreamer/values.yaml#L3618) setting of the Adapter Set definition with the full path of the deployment folder:
...
2. Deploy the Adapter Set's resource to a persistent storage
   ...
   - Configure the [`provisioning.fromVolume`](charts/lightstreamer/values.yaml#L3622) setting of the Adapter Set definition with the reference to the volume and optionally the deployment full path in the volume:
...
- [`metadataProvider.inProcessMetadataAdapter`](charts/lightstreamer/values.yaml#L3723)
- [`dataProviders.<dataProviderName>.inProcessDataAdapter`](charts/lightstreamer/values.yaml#L4398)
...
- `adapterClass` ([Metadata Adapter](charts/lightstreamer/values.yaml#L3723), [Data Adapter](charts/lightstreamer/values.yaml#L4398)): The fully qualified name of the Java class implementing the Adapter.

- `installDir` ([Metadata Adapter](charts/lightstreamer/values.yaml#L3723), [Data Adapter](charts/lightstreamer/values.yaml#L4398)): The directory where the Adapter's own `lib` and `classes` folders are located in the provisioning source. Optional, but mandatory when `classLoader` is set to `dedicated`. The full path is available at `/deployed_adapters/<adapter-set-folder>/<installDir>` in the container.

- `classLoader` ([Metadata Adapter](charts/lightstreamer/values.yaml#L3772), [Data Adapter](charts/lightstreamer/values.yaml#L4398)): The ClassLoader strategy for loading the Adapter's classes. See [ClassLoader types](#classloader-types) for details.

- `configMapRef` ([Metadata Adapter](charts/lightstreamer/values.yaml#L3723), [Data Adapter](charts/lightstreamer/values.yaml#L4398)): An optional reference to a Kubernetes ConfigMap whose files are copied into the adapter's deployment directory at startup. This is useful for injecting adapter-specific configuration files without rebuilding the container image.
...
- `initParams` ([Metadata Adapter](charts/lightstreamer/values.yaml#L3909), [Data Adapter](charts/lightstreamer/values.yaml#L4461)): An optional map of key/value pairs forwarded as-is to the adapter's `init()` method. Use this to pass adapter-specific configuration without hardcoding it into the adapter's code.
...
- [`enableTableNotificationsSequentialization`](charts/lightstreamer/values.yaml#L3897) (Metadata Adapter only): When `true`, all subscription lifecycle notifications for the same session are delivered sequentially with no overlap. Useful when the Metadata Adapter implementation is not designed for concurrent table notifications.

**Advanced: thread pool tuning**

- **Metadata Adapter**:
  - [`authenticationPool`](charts/lightstreamer/values.yaml#L3801): Dedicated thread pool for `notifyUser` calls.
  - [`messagesPool`](charts/lightstreamer/values.yaml#L3847): Dedicated thread pool for `notifyUserMessage` calls.
  - [`mpnPool`](charts/lightstreamer/values.yaml#L3880): Dedicated thread pool for mobile push notification requests.
- **Data Adapter**:
  - [`dataAdapterPool`](charts/lightstreamer/values.yaml#L4440): Dedicated thread pool for subscription/unsubscription management.
...
3. The Lightstreamer Broker's `shared` folder:
   ...
   To populate the `shared` folder, configure the [`sharedDir`](charts/lightstreamer/values.yaml#L3614) section. You can embed the resources in a custom container image:
...
- [`metadataProvider.proxyMetadataAdapter`](charts/lightstreamer/values.yaml#L3920)
- [`dataProviders.<dataProviderName>.proxyDataAdapter`](charts/lightstreamer/values.yaml#L4472)
...
- `requestReplyPort` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4065), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4472)): The mandatory TCP port the Proxy Adapter listens on for the Remote Server to connect.

- `remoteHost` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4087), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4541)): When set, inverts the connection direction — the Proxy Adapter connects out to the Remote Server instead of waiting for an inbound connection. Useful when the Broker cannot accept incoming connections from outside.

- `interface` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4092), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4546)): Binds the Proxy Adapter to a specific local network interface. When not set, the Proxy Adapter binds to all available interfaces.

- `sslConfig` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4096), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4549)): Optional TLS/SSL settings for the connection to the Remote Server. Supports the same keystore/truststore configuration as server sockets. See [`sslConfig`](charts/lightstreamer/values.yaml#L899) for details.

- `authentication` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4140), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4585)): When enabled, enforces credential-based authentication of Remote Server connections. Credentials are referenced from Kubernetes secrets (each containing `user` and `password` keys).
...
- `enableRobustAdapter` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L3920), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4472)): Enables the _Robust_ variant of the Proxy Adapter, which handles the temporary absence of the Remote Server gracefully — accepting subscriptions and waiting for reconnection rather than failing immediately.

- [`enableTableNotificationsSequentialization`](charts/lightstreamer/values.yaml#L4061) (Proxy Metadata Adapter only): When `true`, all subscription lifecycle notifications for the same session are delivered sequentially with no overlap. Useful when the Metadata Adapter implementation is not designed for concurrent table notifications.

- `connectionRecoveryTimeoutMillis` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4170), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4615)): Only effective when `enableRobustAdapter` is set. After a failed connection attempt, the Proxy Adapter waits at least this long before retrying. A negative value prevents further attempts.

- `firstConnectionTimeoutMillis` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4181), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4629)): Only effective when `enableRobustAdapter` is set. How long the Broker startup may be delayed waiting for the first Remote Server connection. A negative value means unlimited.

**Advanced: thread pool tuning and connection settings**

- **Proxy Metadata Adapter**: [`authenticationPool`](charts/lightstreamer/values.yaml#L3966), [`messagesPool`](charts/lightstreamer/values.yaml#L4011), [`mpnPool`](charts/lightstreamer/values.yaml#L4044) — same tuning options as for In-Process Metadata Adapters.
- **Proxy Data Adapter**: [`dataAdapterPool`](charts/lightstreamer/values.yaml#L4507) — dedicated thread pool for subscription/unsubscription management (`maxSize`, `maxFree`).
- `connectionRetryMillis` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4159), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4604)), `keepaliveTimeoutMillis` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4310), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4751)), `keepaliveHintMillis` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4310), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4751)), `timeoutMillis` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4310), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4751)): Connection reliability settings.

- `remoteAddressWhitelist` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4349), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4763)): Comma-separated list of hosts allowed to connect as Remote Adapters. When not set, any host is accepted.

- `remoteParamsConfig` ([Proxy Metadata Adapter](charts/lightstreamer/values.yaml#L4271), [Proxy Data Adapter](charts/lightstreamer/values.yaml#L4713)): Custom initialization parameters to forward to the Remote Adapter on connection. Uses a `prefix` to select which parameters to send, plus a `params` map of key/value pairs.
...
See the [`adapters.welcomeAdapterSet`](charts/lightstreamer/values.yaml#L4798) section of `values.yaml` for full details.
...
Currently, the Kafka Connector is the only connector available in this Helm chart. Each connector is configured and enabled independently within the [`connectors`](charts/lightstreamer/values.yaml#L4871) section.
...
To configure the Kafka Connector, define its settings in the [`connectors.kafkaConnector`](charts/lightstreamer/values.yaml#L4874) section:
...
The Kafka Connector must be provisioned before it can be used. The Helm chart supports multiple provisioning methods through the [`provisioning`](charts/lightstreamer/values.yaml#L4883) section:
...
The [`adapterSetId`](charts/lightstreamer/values.yaml#L4920) setting defines the unique Adapter Set ID for the Kafka Connector. Clients use this value when establishing a connection to the Lightstreamer Server through a `LightstreamerClient` object.

The [`adapterClassName`](charts/lightstreamer/values.yaml#L4928) setting specifies the Java class of the Kafka Connector Metadata Adapter. The default value (`com.lightstreamer.kafka.adapters.pub.KafkaConnectorMetadataAdapter`) is suitable for most deployments. To customize authentication, authorization, or item validation logic, provide a subclass of the factory class — see [Customizing the Kafka Connector Metadata Adapter class](https://github.com/Lightstreamer/Lightstreamer-kafka-connector/tree/main?tab=readme-ov-file#customizing-the-kafka-connector-metadata-adapter-class) for details.

##### Logging

Configure Kafka Connector logging through the [`logging`](charts/lightstreamer/values.yaml#L4931) section. The configuration follows a two-part model: **appenders** define _where_ and _how_ log output is written, while **loggers** control _what_ gets logged and at which level.

**Appenders** are defined in the [`appenders`](charts/lightstreamer/values.yaml#L4936) map. Each appender has a `type` — either `Console` (writes to standard output) or `DailyRollingFile` (writes to a daily-rotated file). Both types require a `pattern` string using [reload4j `PatternLayout`](https://reload4j.qos.ch/apidocs/org/apache/log4j/PatternLayout.html) syntax. File appenders additionally require `fileName`, `fileNamePattern`, and optionally `volumeRef` (referencing a volume defined in `deployment.extraVolumes`) to persist logs outside the container:
...
**Loggers** are defined in the [`loggers`](charts/lightstreamer/values.yaml#L4974) map. Each entry is keyed by a fully qualified class name or package and specifies a `level` and one or more `appenders` references:
...
The Kafka Connector supports multiple independent connections to different Kafka brokers or clusters. Each connection is defined in the [`connections`](charts/lightstreamer/values.yaml#L5008) map and must set `enabled: true` to be active (disabled connections automatically deny all subscription requests):
...
**Connection name**: Each connection must have a unique [`name`](charts/lightstreamer/values.yaml#L5028). Clients use this value when subscribing to request real-time data from a specific Kafka connection.

**Bootstrap servers**: Specify one or more Kafka broker addresses using [`bootstrapServers`](charts/lightstreamer/values.yaml#L5034). For Kafka deployed in Kubernetes, use the service DNS name:

```yaml
bootstrapServers: "kafka-0.kafka-headless.kafka:9092"
```

**Consumer group**: The optional [`groupId`](charts/lightstreamer/values.yaml#L5057) sets the Kafka `group.id` for the internal consumer. When not specified, the connector generates a default value from `adapterSetId`, the connection name, and a random suffix.
...
**Encryption**: Enable TLS/SSL encryption for the connection through [`sslConfig`](charts/lightstreamer/values.yaml#L5060). The `truststoreRef` validates broker certificates, and `keystoreRef` supplies a client certificate when mutual TLS is required. Both reference entries defined in the [Keystores](#keystores) section:
...
**Authentication**: Configure SASL authentication for the connection through [`authentication`](charts/lightstreamer/values.yaml#L5099). The supported mechanisms are:

- `PLAIN`, `SCRAM-SHA-256`, `SCRAM-SHA-512`: username/password authentication. Supply credentials via a Kubernetes Secret referenced by [`credentialsSecretRef`](charts/lightstreamer/values.yaml#L5120) (must contain `user` and `password` keys).
- `GSSAPI`: Kerberos authentication. Requires [`gssapi`](charts/lightstreamer/values.yaml#L5124) settings (service name, principal, optional keytab).
- `AWS_MSK_IAM`: IAM-based authentication for Amazon MSK. Optionally configure a credential profile, role ARN, and STS region through the [`iam`](charts/lightstreamer/values.yaml#L5150) block.
...
**Record processing**: The [`record`](charts/lightstreamer/values.yaml#L5174) block controls how Kafka messages are consumed and deserialized.

[`consumeFrom`](charts/lightstreamer/values.yaml#L5187) (default: `LATEST`) sets the initial offset — use `EARLIEST` to replay all existing messages on first connection.
...
[`consumeWithThreadNumber`](charts/lightstreamer/values.yaml#L5219) (default: `1`) controls parallelism for processing deserialized records. When using more than one thread, [`consumeWithOrderStrategy`](charts/lightstreamer/values.yaml#L5230) determines ordering guarantees: `ORDER_BY_PARTITION` (default), `ORDER_BY_KEY`, or `UNORDERED`.

[`keyEvaluator`](charts/lightstreamer/values.yaml#L5234) and [`valueEvaluator`](charts/lightstreamer/values.yaml#L5290) configure how message keys and values are deserialized. Supported types:
...
Routing configuration maps Kafka topics to Lightstreamer items. Define routing rules in the [`routing`](charts/lightstreamer/values.yaml#L5348) section:
...
Field mapping defines how Kafka message content is transformed into Lightstreamer fields. Configure mappings in the [`fields`](charts/lightstreamer/values.yaml#L5403) section:
...
Set [`enableSkipFailedMapping`](charts/lightstreamer/values.yaml#L5429) to `true` to continue processing even if some field extractions fail.

##### Connection-specific logging

Each connection can override the global logging configuration using the [`logger`](charts/lightstreamer/values.yaml#L5476) setting:
...
**Local schema files**: Define named schema references in the [`localSchemaFiles`](charts/lightstreamer/values.yaml#L5488) map. Each entry points to a ConfigMap name and key containing the schema file (`.avsc` for Avro, `.json` for JSON Schema, `.proto` or binary descriptor for Protobuf):
...
3. Reference the ConfigMap in `localSchemaFiles` and configure the evaluator with both `localSchemaFilePathRef` and [`protobufMessageType`](charts/lightstreamer/values.yaml#L5274):
...
**Schema Registry**: Define named registry configurations in the [`schemaRegistries`](charts/lightstreamer/values.yaml#L5504) map. Two providers are supported:

- `CONFLUENT`: requires a [`url`](charts/lightstreamer/values.yaml#L5520). Optional basic HTTP authentication and TLS settings are available under the [`confluent`](charts/lightstreamer/values.yaml#L5524) block (TLS configuration becomes mandatory when the URL uses the `https` protocol).
- `AZURE`: supports JSON and AVRO only (not Protobuf). Requires a [`url`](charts/lightstreamer/values.yaml#L5520) pointing to the Azure Event Hubs namespace (e.g., `https://my-namespace.servicebus.windows.net`) and a credentials secret (containing `client_id`, `tenant_id`, and `client_secret` keys) referenced by [`azure.credentialsSecretRef`](charts/lightstreamer/values.yaml#L5536).
...
Then enable the Schema Registry on the evaluator and reference the registry at the connection level via [`schemaRegistryRef`](charts/lightstreamer/values.yaml#L5345):
...
See the [`connectors.kafkaConnector`](charts/lightstreamer/values.yaml#L4874) section of `values.yaml` for full details.
