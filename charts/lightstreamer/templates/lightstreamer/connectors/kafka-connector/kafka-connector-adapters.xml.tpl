    ...
    <data_provider name={{ $connection.name | quote }}>
        ...
        <!-- Mandatory if evaluator type is AVRO and the Confluent Schema Registry is disabled. The path of the local schema
            ...
            {{- with .localSchemaFilePathRef }}
              {{ $localSchema := required (printf "connectors.kafkaConnector.localSchemaFiles.%s not defined" . ) (get ($.Values.connectors.kafkaConnector.localSchemaFiles | default dict) .) }}
